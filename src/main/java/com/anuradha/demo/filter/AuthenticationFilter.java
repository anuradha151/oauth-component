package com.anuradha.demo.filter;


import com.anuradha.demo.annotation.Secured;
...
public class AuthenticationFilter implements ContainerRequestFilter {
    ...
    public void filter(final ContainerRequestContext requestContext) throws IOException {
        ...
        if (authorizationHeader == null || !authorizationHeader.startsWith("Bearer "))
            throw new PMException("Authorization headers are required.");
    ...
    private Claims validateToken(String token, String key) {
        try {
            ...
            throw new PMException("Unauthorized request.");
        }
    }
    ...
    private void checkPermissions(Set<String> allowedRoles, String role) {
        if (!allowedRoles.contains(role))
            throw new PMException("Unauthorized user.");
    }
}
